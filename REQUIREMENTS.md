...
| **PyPI Release Readiness** | High | Mixed | PyPI publication remains blocked. Local artifact readiness checks, CPU-first dependency behavior, backend setup commands, and Ubuntu clean artifact CI now exist, but uninstall/reinstall smoke coverage, package metadata review, TestPyPI validation, publication policy, and cross-platform support matrix completion are still required. |
