...
| **TestPyPI and Publication Policy** | Medium | Mixed | Added a TestPyPI publication policy document and manual `.github/workflows/phase13-testpypi.yml` workflow protected by the `testpypi` environment. It verifies built versions, publishes through OIDC trusted publishing without `skip-existing`, verifies a clean TestPyPI install, and retains versioned evidence artifacts. The TestPyPI `1.0.0` candidate has succeeded; production PyPI remains blocked until CUDA hardware evidence and repository visibility/publication policy are approved. |
...
| **TestPyPI Publication Evidence** | Medium | Done | `.github/workflows/phase13-testpypi.yml` successfully published and clean-installed the `1.0.0` candidate through the protected OIDC path. Retained `phase13-testpypi-1.0.0` evidence records the candidate; production publication remains separately blocked. |
...
3. CI and release automation: Done for fast PR CI, clean artifact smoke, CI documentation, and retained cross-platform matrix evidence. The TestPyPI `1.0.0` gate has passed; the CUDA workflow remains a manual release gate until retained real-hardware evidence passes. See `docs/reference/ci-workflows.md`.
...
6. TestPyPI and publication policy:
   ...
   - Done: the protected OIDC workflow published and clean-installed the TestPyPI `1.0.0` candidate. Retain its evidence artifact and use its version-pinned dual-index command only as a release-candidate installation path.
