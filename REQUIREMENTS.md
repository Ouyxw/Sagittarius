...
| **Production Promotion and Failure Evidence** | High | Mixed | The candidate, clean-install, matrix, TestPyPI, and CUDA workflows upload status, logs, identities, manifests, and available partial diagnostics with `if: always()`. A separately protected production PyPI workflow, exact-file promotion, and pinned production-index post-install smoke remain planned. |
