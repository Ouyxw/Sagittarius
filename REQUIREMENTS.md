...
| **Cross-Platform Installation Matrix** | High | Mixed | Added `docs/getting-started/python/compatibility-matrix.md` and manual `.github/workflows/phase13-cross-platform.yml` covering Linux Python 3.10/3.11/3.12 with Julia 1.10.3/1.11 plus macOS and Windows CPU wheel smokes on Python 3.11/Julia 1.11. The workflow uploads per-row evidence artifacts, but release validation still requires a successful run and retained evidence for all matrix rows. |
| **CI Clean Artifact Isolation** | High | Done | Adds `.github/workflows/phase13-clean-artifact.yml`, which runs the clean wheel and uninstall/reinstall smokes on Ubuntu from a fresh checkout with `PYTHONPATH` cleared. The workflow runs automatically only on relevant pushes to `main` and manually on demand, while day-to-day PRs use `.github/workflows/pr-fast-ci.yml`. The smokes build wheel/sdist artifacts, install the wheel into fresh venvs outside the repository, run installed `sagittarius backend resolve`, execute CPU one-atom simulations, validate doctor/version/result metadata, verify uninstall removes the import, and fail if `sagittarius` imports from the source checkout. |
...
3. CI clean artifact isolation: Done. Fast PR CI handles lightweight docs/metadata/artifact checks for day-to-day pull requests. Ubuntu clean artifact CI runs the clean wheel and uninstall/reinstall smokes from a fresh checkout on relevant pushes to `main` or manually on demand, clears source-tree import paths, installs the wheel into repo-external venvs, runs `sagittarius backend resolve`, validates CPU simulation artifacts and metadata, verifies uninstall removes the package import, and fails on source-checkout imports.
