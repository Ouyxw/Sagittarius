...
| **Release Candidate Governance** | High | Mixed | `SPEC-GOV-006` defines `main`-based lineage, immutable identity, package boundaries, build-once promotion, and CUDA evidence timing. The canonical candidate, identity, artifact reuse, content, sdist, digest, and current-gate failure-evidence controls are implemented; full regression evidence, final hardware/service executions, and production promotion remain open. |
...
3. CI and release automation: Done for fast PR CI, canonical build-once distributions, ref/version/`main` identity enforcement, digest-verified reuse by the current release gates, wheel/sdist forbidden-content checks, clean retained-sdist installation, current-gate failure evidence, clean artifact smoke, CI documentation, and historical cross-platform evidence. Remaining work is complete Python and Julia-native release regression, final-candidate TestPyPI/cross-platform/real-hardware CUDA execution evidence, protected production promotion, and post-publication smoke. See `docs/reference/ci-workflows.md`.
