...
| **Package Metadata Review** | Medium | Done | PyPI-facing metadata now includes README rendering, MIT license expression and license file inclusion, production/stable and supported Python classifiers, Julia/science classifiers, author field, keywords, project URLs, console script metadata, and artifact content checks. Packaging tests inspect wheel/sdist metadata and run `twine check` over built artifacts. |
| **TestPyPI and Publication Policy** | Medium | Mixed | Added a TestPyPI publication policy document and manual `.github/workflows/phase13-testpypi.yml` workflow protected by the `testpypi` environment. It verifies built versions, publishes through OIDC trusted publishing without `skip-existing`, verifies a clean TestPyPI install, and retains versioned evidence artifacts. Production PyPI remains blocked until the workflow succeeds and repository visibility/publication policy is approved. |
...
| **TestPyPI Publication Evidence** | Medium | Mixed | `.github/workflows/phase13-testpypi.yml` is a manual, `testpypi`-environment-protected OIDC release gate. It requires a new candidate version and retains a `phase13-testpypi-<version>` artifact; successful execution and clean install evidence remain required. |
...
6. TestPyPI and publication policy:
   ...
   - Configure TestPyPI trusted publishing, run the protected OIDC workflow with a new candidate version, and verify install from TestPyPI in a clean environment before production PyPI.
