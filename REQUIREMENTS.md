...
| **PyPI Release Readiness** | High | Mixed | The current canonical candidate passed candidate-artifact, regression, clean-artifact, cross-platform, TestPyPI, and CUDA-wheel gates. Production publication remains blocked pending a separately protected exact-file promotion workflow, production digest reconciliation, and a clean production-index smoke. CUDA remains experimental. |
...
| **Production Promotion and Failure Evidence** | High | Mixed | The candidate, clean-install, matrix, TestPyPI, and CUDA workflows upload status, logs, identities, manifests, and available partial diagnostics with `if: always()`. A separately protected production PyPI workflow, exact-file promotion, production digest reconciliation, and pinned production-index post-install smoke remain required. |
