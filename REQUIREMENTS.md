...
| **CI Clean Artifact Isolation** | High | Done | Adds `.github/workflows/phase13-clean-artifact.yml`, which runs the clean wheel and uninstall/reinstall smokes on Ubuntu from a fresh checkout with `PYTHONPATH` cleared. The smokes build wheel/sdist artifacts, install the wheel into fresh venvs outside the repository, run installed `sagittarius backend resolve`, execute CPU one-atom simulations, validate doctor/version/result metadata, verify uninstall removes the import, and fail if `sagittarius` imports from the source checkout. |
...
3. CI clean artifact isolation: Done. Ubuntu CI runs the clean wheel and uninstall/reinstall smokes from a fresh checkout, clears source-tree import paths, installs the wheel into repo-external venvs, runs `sagittarius backend resolve`, validates CPU simulation artifacts and metadata, verifies uninstall removes the package import, and fails on source-checkout imports.
