...
Phase 13 planned work includes:

- relocatable wheel containing Julia backend sources and project metadata;
- installed-package resource lookup instead of repository-relative backend discovery;
- CPU-first dependency profile that does not require CUDA for regular CPU users;
