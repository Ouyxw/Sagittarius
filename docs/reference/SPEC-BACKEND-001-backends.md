...
The default `sagittarius_py/juliapkg.json` environment is CPU-first and intentionally excludes CUDA.jl. CUDA setup is explicit through the packaged `sagittarius_py/sagittarius/juliapkg-cuda.json` profile and `sagittarius backend install cuda`; the gated clean-wheel CUDA smoke and the devcontainer remain the CUDA parity paths for hardware-backed validation. The devcontainer must keep JuliaPkg and JuliaCall configuration aligned: `.devcontainer/setup-juliacall-env.sh` dynamically derives the Julia executable, JuliaPkg project, and Julia `Sys.BINDIR`, then writes the complete `PYTHON_JULIACALL_EXE`/`PYTHON_JULIACALL_PROJECT`/`PYTHON_JULIACALL_BINDIR` set into `/etc/profile.d/sagittarius-juliacall.sh`. This avoids hard-coding Juliaup versioned runtime paths and prevents partial JuliaCall environments from masking CUDA diagnostics as Julia startup failures. The planned AMDGPU/Metal packages currently introduce dependency constraints that can prevent CUDA.jl 6.2.x from resolving. Use a separate backend-specific Julia environment when experimenting with AMDGPU or Metal.
