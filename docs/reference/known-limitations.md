...
Sagittarius 1.0.11 is the current stable research SDK release. This page records practical limits and unsupported scenarios so examples, benchmarks, and hardware-facing studies can be interpreted correctly.

## API Stability

- Sagittarius 1.0.11 defines the current supported Python SDK and Julia developer API surface. Future incompatible changes require a documented compatibility and versioning decision under [`SPEC-GOV-005-repository-versioning.md`](../governance/SPEC-GOV-005-repository-versioning.md).
...
- The Phase 16 CUDA/MWIS protocol is disabled unless `SAGITTARIUS_ENABLE_GPU_TESTS=1`; it runs only after initialized CUDA doctor diagnostics succeed. Its small CPU/CUDA rows are local diagnostic evidence and do not establish GPU performance, scale limits, or solver quality on another host.
- The retained [Phase 16 closure archive](../benchmarks/evidence/phase16-e171c584e8e67215f1e5fb00f6f908d2e434b10c/README.md) is reviewable local evidence on one RTX 5070 Ti host; it does not relax these CUDA limitations.
...
- Phase 16 MWIS/UDG AQC evidence is limited to deterministic CPU instances of at most six nodes with exact PuLP/CBC references. Its bounded scaling rows preserve scale metadata and failures, but do not establish general solver quality, asymptotic scaling, or hardware advantage.
- Phase 16 `ParallelSimulation` sweep evidence uses local Julia `Distributed` workers and a deterministic parameter-map kernel. It does not validate multi-node scheduling, MPI/Slurm deployment, or end-to-end physics-solver sweep scalability.
- `experiment-config/v1` reproduces JSON-declared register, pulse, solver, observable, readout, and basis-state inputs only. Python callable pulses, arbitrary complex initial-state vectors, named helper forms, parameter sweeps, and hardware-calibration inputs remain outside this contract. The Python SDK provides all-ground, bitstring, and single-excitation helpers for direct simulation calls; uniform-superposition preparation remains unsupported.
...
- Phase 16 open-system benchmark coverage is restricted to a one-atom CPU correctness case with local decay/dephasing; it does not establish multi-atom decoherence behavior, trajectory scaling, or hardware performance.
