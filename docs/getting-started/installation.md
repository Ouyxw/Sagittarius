...
Sagittarius `1.0.11` is available on production PyPI. Python users should install it in a virtual environment with `python -m pip install sagittarius-py==1.0.11`; a complete repository checkout remains the supported path for contributors and source development.
...
| Run a small external-project workflow from an installed Python wheel. | [Wheel-installed Python recipe](python/wheel-experiment-recipe.md) |
...
Phase 13 packaging embeds the Julia backend sources and project metadata in Python artifacts. The MIT TestPyPI `1.0.8` candidate remains historical evidence and cannot be promoted after the Apache-2.0 licensing decision. Apache-2.0 `1.0.11` passed the candidate, regression, clean-artifact, cross-platform, TestPyPI, CUDA-wheel, and protected production-promotion gates; production-file hashes were reconciled and a clean production-index smoke was retained. CUDA remains experimental. The embedded copy does not replace `Sagittarius.jl` as the native Julia entry point.
