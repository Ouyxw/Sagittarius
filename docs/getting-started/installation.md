...
Phase 13 packaging now embeds the Julia backend sources and project metadata in Python artifacts. Remaining release-readiness work still covers CI clean artifact isolation, uninstall/reinstall smoke coverage, cross-platform artifact tests, and PyPI publication gates. The embedded copy does not replace `Sagittarius.jl` as the native Julia entry point.
