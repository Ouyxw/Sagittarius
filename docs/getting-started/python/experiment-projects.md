...
Editable experiment environments load the adjacent `Sagittarius.jl` checkout first, so Julia backend edits in the repository take effect without rebuilding a wheel. The editable dependency also points to the local checkout for Python source updates, so moving the checkout requires updating the uv source path and running `uv sync` again. Released wheels use `package_resource` when installed away from the checkout. Experiments that do not modify Sagittarius can instead install `sagittarius-py==1.0.10` from PyPI; see [Python package installation](package-installation.md).
