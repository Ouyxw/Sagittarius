...
This matrix records the Phase 13 validation completed for the published `1.0.11` wheel and source distribution. CUDA remains experimental even though it passed validation on a separate CUDA runner.

## Supported CPU Artifact Matrix

| Operating system | Python | Julia | Status | Validation command | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Linux x86_64 | 3.10 | 1.10.3 | Validated for 1.0.11 | `SAGITTARIUS_RUN_RELEASE_ARTIFACT_SMOKE=1 uv run python -m pytest tests/test_packaging_artifacts.py::test_clean_venv_installed_wheel_release_smoke -q` | Minimum declared Python and Julia baseline. |
| Linux x86_64 | 3.11 | 1.11 | Validated for 1.0.11 | Same clean wheel smoke | Primary development and container baseline. |
| Linux x86_64 | 3.12 | 1.11 | Validated for 1.0.11 | Same clean wheel smoke | Newest declared Python line. |
| macOS x86_64/arm64 runner | 3.11 | 1.11 | Validated for 1.0.11 | Same clean wheel smoke | CUDA is not supported on macOS; Metal remains planned/experimental outside the default package profile. |
| Windows x86_64 | 3.11 | 1.11 | Validated for 1.0.11 | Same clean wheel smoke | Path handling and Julia executable discovery passed the clean wheel smoke. |

## Policy

- The default CPU install path must not require CUDA.jl, NVIDIA drivers, or GPU hardware.
- The MIT `1.0.8` candidate remains historical evidence. Apache-2.0 `1.0.11` passed every declared matrix row before production publication.
