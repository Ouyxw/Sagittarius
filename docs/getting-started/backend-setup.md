...
This guide covers Julia executable discovery, JuliaPkg resolution, and the current CPU/GPU backend boundary. It applies to source checkouts and editable experiment projects.
...
CUDA requires a compatible host driver, device passthrough if containerized, CUDA runtime compatibility, Julia CUDA packages, and a visible GPU device. See [Backend maturity](../reference/backends.md) and [Containerized development](containerization.md).
