...
**Q: `uv run python check_env.py` 报 JuliaCall/Julia failed 怎么办？**
...
exec bash -l
```
然后重新执行 `cd sagittarius_py && uv run python check_env.py`。不要只设置 `PYTHON_JULIACALL_EXE`；JuliaCall 要求 `PYTHON_JULIACALL_EXE` 和 `PYTHON_JULIACALL_PROJECT` 成对出现，`PYTHON_JULIACALL_BINDIR` 应由 Julia `Sys.BINDIR` 计算，而不是简单使用 Juliaup shim 所在目录。
