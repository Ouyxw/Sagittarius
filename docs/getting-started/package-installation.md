...
This is a local release-readiness gate, not PyPI publication approval. PyPI remains blocked until uninstall/reinstall smoke tests, successful TestPyPI execution, GPU runner evidence, and the cross-platform support matrix are complete. Package metadata checks, Ubuntu clean artifact CI, a TestPyPI workflow, and a gated CUDA wheel smoke are in place; broader matrix coverage remains planned.
