...
The payload fields follow `docs/EVENT_TAXONOMY.md` and can be captured with Julia's standard `Logging` tools. Native solver calls emit `solver_start` and `solver_finish`; `setup_workers` emits `cluster_setup_start` and `cluster_setup_finish`; basis and Hamiltonian builders emit their physics events.
