...
| **PyPI Release Readiness** | High | Mixed | The current canonical candidate passed candidate-artifact, regression, clean-artifact, cross-platform, TestPyPI, and CUDA-wheel gates. The protected exact-file promotion workflow is implemented but remains unexecuted; production publication stays blocked pending a newly frozen candidate, production digest reconciliation, and a clean production-index smoke. CUDA remains experimental. |
...
| **Release Candidate Governance** | High | Mixed | `SPEC-GOV-006` defines `main`-based lineage, immutable identity, package boundaries, build-once promotion, and CUDA evidence timing. The canonical candidate, identity, artifact reuse, content, sdist, digest, and current-gate failure-evidence controls are implemented; implementation of full regression workflows is complete; final candidate executions, final hardware/service evidence, and production promotion remain open. |
...
| **Release Regression Coverage** | High | Mixed | The canonical-candidate release-regression workflow runs backend-free and Julia-backed Python suites plus native Julia `Pkg.test()`. Final-candidate execution evidence remains required before release approval. |
...
| **Production Promotion and Failure Evidence** | High | Mixed | The candidate, clean-install, matrix, TestPyPI, and CUDA workflows upload status, logs, identities, manifests, and available partial diagnostics with `if: always()`. The separately protected production workflow is implemented; its first approved execution must still provide exact-file promotion, production digest reconciliation, and a pinned production-index post-install smoke. |
...
3. CI and release automation: Done for fast PR CI, canonical build-once distributions, ref/version/`main` identity enforcement, digest-verified reuse by the current release gates, wheel/sdist forbidden-content checks, clean retained-sdist installation, current-gate failure evidence, clean artifact smoke, CI documentation, and historical cross-platform evidence. Remaining work is final-candidate release-regression, TestPyPI/cross-platform/real-hardware CUDA execution evidence, protected production promotion, and post-publication smoke. See `docs/reference/ci-workflows.md`.
...
6. TestPyPI and publication policy:
   ...
   - Approve repository visibility, issue tracker/docs readiness, Apache-2.0 license publication, and accidental-upload controls before production release.
