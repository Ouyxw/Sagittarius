...
| **Solver and Backend Performance Benchmarks** | High | Mixed | Existing scaling, GPU, cluster, and ablation runners emit structured rows while preserving legacy metrics. The opt-in CUDA protocol now records CPU/CUDA error, cold/warm execution, GPU memory, and doctor-gated failure boundaries; solver-method sensitivity and broader scenario standardization remain planned. |
