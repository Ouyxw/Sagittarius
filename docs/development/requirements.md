...
| **Benchmark Artifact Contracts** | High | Mixed | `benchmark-artifact/v1` now supports an optional structured single-scenario row profile with case IDs, seeds/configuration, backend diagnostics, correctness/performance metrics, result/manifest links, disclosure status, and structured failure rows. The aggregate `benchmark-suite-artifact/v1` wrapper remains planned. |
