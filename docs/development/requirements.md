...
| **Physics Baseline Benchmarks** | High | Mixed | Runnable CPU smoke and correctness cases cover single-atom Rabi, ideal two-atom blockade, Landau-Zener sweep, and a small-chain projected dense-vs-reduced reference. Correctness rows retain stable tolerances, reference errors, and result/manifest or reference-report links. |
| **Cold-Atom Dynamics Benchmarks** | Medium | Mixed | Correctness runner covers 3-atom global-chain excitation, 3-atom local addressing, 4-atom Z2 chain, and 2x2 2D array cases against projected dense matrix-exponential references (atol `1e-8`); larger trajectory, correlation, and performance studies remain planned. |
| **Open-System Benchmarks** | Medium | Mixed | Seeded one-atom decay plus dephasing rows validate analytic population decay, Lindblad trace/positivity, MCWF-vs-Lindblad agreement, and 100/500-trajectory sensitivity (500 no worse); multi-atom decoherence-impact and scaling studies remain planned. |
| **Optimization Benchmark Suite** | High | Mixed | CPU `smoke`, `correctness`, and bounded `scaling` tiers now run deterministic weighted UDG/MWIS AQC cases with graph/weight/schedule metadata, exact PuLP/CBC baselines, feasibility, objective gap, approximation ratio, optimal-success probability, reference reports, suite artifacts, and retained failure rows. Broader schedules, structured/planted families, and governed performance conclusions remain planned. |
| **MWIS/UDG GPU Benchmark Protocol** | High | Mixed | Opt-in `benchmark_cuda_mwis_protocol.py` requires `SAGITTARIUS_ENABLE_GPU_TESTS=1` and successful initialized CUDA doctor diagnostics before executing CPU/CUDA chain and seeded MWIS parity. It retains device/driver/CUDA/Julia metadata, cold/warm timing, GPU-memory snapshots, errors, exact MWIS feasibility/objective checks, and doctor/solver failure boundaries. Hardware-specific scale limits and governed performance conclusions remain planned. |
| **Solver and Backend Performance Benchmarks** | High | Mixed | Existing scaling, GPU, cluster, and ablation runners emit structured rows while preserving legacy metrics. `benchmark_solver_performance.py` now repeats dense/sparse/reduced path measurements with declared numerical gates and validates Tsit5/Vern9/RK4 trajectories against a high-accuracy Tsit5 reference; the CUDA protocol records CPU/CUDA error, cold/warm execution, GPU memory, and doctor-gated failure boundaries. Broader scenario standardization remains planned. |
| **Parameter Sweep and Cluster Benchmarking** | Medium | Mixed | `benchmark_sweep_cluster.py` records local `ParallelSimulation` parameter-map throughput, checkpointed `sweep-artifact/v1` state, per-item result/manifest links, failure rows, resume-only pending execution, and aggregate suite evidence. Multi-node deployment and physics-workload sweep coverage remain planned. |
| **Benchmark Failure Reporting** | High | Mixed | `make_benchmark_failure_row()` records first-class timeout, memory, CUDA initialization, reference mismatch, ILP, and unexpected-execution failure rows with exception type, stage, diagnostics, timeout, process/GPU memory, backend probe, and remediation. Runner-wide timeout and scale-limit policies remain planned. |
