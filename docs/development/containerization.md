...
- **JuliaCall 启动配置**: `postCreateCommand` 会运行 `.devcontainer/setup-juliacall-env.sh`，根据容器内实际的 `julia` 可执行文件、`PYTHON_JULIAPKG_PROJECT` 和 Julia `Sys.BINDIR` 动态生成 `/etc/profile.d/sagittarius-juliacall.sh`。这会成对设置 `PYTHON_JULIACALL_EXE`、`PYTHON_JULIACALL_PROJECT` 和 `PYTHON_JULIACALL_BINDIR`，避免 Juliaup shim 路径或只设置单个 JuliaCall 变量导致 `juliacall` 初始化失败。
...
### 4.1 环境自检工具
在容器内使用正式诊断命令检查 CUDA 后端。该命令输出结构化 JSON；`--initialize-backend` 会额外初始化 Julia 并执行后端探测：
```bash
cd sagittarius_py
uv run sagittarius doctor --backend CUDA --initialize-backend
```
...
**Q: `uv run sagittarius doctor --backend CUDA --initialize-backend` 报 JuliaCall/Julia failed 怎么办？**
A: 先确认新 shell 中存在完整的 JuliaCall 三件套：
```bash
env | grep PYTHON_JULIACALL
```
正常情况下，devcontainer 的 `postCreateCommand` 已经通过 `.devcontainer/setup-juliacall-env.sh` 生成 `/etc/profile.d/sagittarius-juliacall.sh`。如果容器是在脚本加入前创建的，运行：
```bash
cd /workspaces/Sagittarius
.devcontainer/setup-juliacall-env.sh
exec bash -l
```
然后重新执行 `cd sagittarius_py && uv run sagittarius doctor --backend CUDA --initialize-backend`。不要只设置 `PYTHON_JULIACALL_EXE`；JuliaCall 要求 `PYTHON_JULIACALL_EXE` 和 `PYTHON_JULIACALL_PROJECT` 成对出现，`PYTHON_JULIACALL_BINDIR` 应由 Julia `Sys.BINDIR` 计算，而不是简单使用 Juliaup shim 所在目录。
