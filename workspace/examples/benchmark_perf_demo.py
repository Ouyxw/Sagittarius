...
def example_solver_comparison():
    ...
    print(f"  • Compared {len(artifacts)} solvers")
    print(f"  • Best performer: {artifacts[0]['solver_name']} ({artifacts[0]['metric_value']:.2f}s)\n")
...
def example_success_failure_summary():
    ...
    success_count = sum(1 for r in artifacts if r['status'] == 'success')
